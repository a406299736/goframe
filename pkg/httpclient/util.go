...
func doHTTP(ctx context.Context, method, url string, success []byte, opt *option) ([]byte, int, error) {
	ts := time.Now()
	...
	req, err := http.NewRequestWithContext(ctx, method, url, bytes.NewReader(success))
	if err != nil {
